...
void AudioMixer_queueSound(wavedata_t *pSound)
{
    ...
    pthread_mutex_lock(&audioMutex);
	{
		...
		if (noFreeSlot)
		{
			printf("ERROR: there are no free slots in soundBites.\n");
		}
	}
...
static void fillPlaybackBuffer(short *buff, int size)
{
	...
	for (int i = 0; i < MAX_SOUND_BITES; i++)
	{
		if (soundBites[i].pSound != NULL)
		{
			pthread_mutex_lock(&audioMutex);
			{
				int indexToStart = soundBites[i].location;
				for (int j = 0; j < size; j++)
				{
					indexToStart++;
					if (indexToStart < soundBites[i].pSound->numSamples) {
						tempBuff[j] += soundBites[i].pSound->pData[indexToStart];
					} else {
						break;
					}
				}
				...
				if (indexToStart >= soundBites[i].pSound->numSamples) {
					...
					soundBites[i].location = indexToStart;
				}
			}
			pthread_mutex_unlock(&audioMutex);
	...
	{
		if (tempBuff[i] > SHORT_MAX) {
			buff[i] = SHORT_MAX;
		} else if (tempBuff[i] < SHORT_MIN) {
			buff[i] = SHORT_MIN;
		} else {
			buff[i] = tempBuff[i];
		}
